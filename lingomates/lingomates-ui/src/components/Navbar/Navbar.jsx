...
import {AppBar, Box, Toolbar, Typography, Button, Grid, Container, IconButton, Avatar} from "@mui/material"
    ...
    return (
        <div className="navbar">
            <header className="header w-full h-auto bg-transparent overflow-x-hidden fixed z-50 top-0 left-0">
                <nav className={`w-full md:h-24 h-20 bg-red lg:px-24 md:px-12 px-8 flex justify-between items-center`}>
                    ...
                    <div className="nav-links flex flex-grow">
                        <ul className="flex">
                            <Link to="/community" className="link-button">
                                <li className="w-full"><button type="button" className="communuity">Community</button></li>
                            </Link>
                            <Link to="/conversations" className="link-button">
                                <li className="w-full"><button type="button" className="convo">Conversations</button></li>
                            </Link>
                            <Link to="/feed" className="link-button">
                                <li className="w-full"><button type="button" className="feed">News</button></li>
                            </Link>
                            {loggedIn && (
                                <>
                                    <Avatar src={currentUser.profilepicture} onClick={getUserProfile} sx={{ width: 50, height: 50, marginLeft: 80, marginRight: -10, cursor: "pointer" }} />
                                    <li onClick={getUserProfile} className="first-name">{currentUser.first_name}</li>
                                    <li onClick={handleLogout} className="w-full-border">
                                        <button type="button" className="loggout" onClick={handleLogout}>Log Out</button>
                                    </li>
                                </>
                            )}
                            {!loggedIn && (
                                <>
                                    <Link to="/login" className="link-button">
                                        <li className="w-full"><button type="button" className="signin">Sign In</button></li>
                                    </Link>
                                </>
                            )}
                        </ul>
